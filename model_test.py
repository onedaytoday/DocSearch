...
class MyTestCase(unittest.TestCase):
    ...
    def test_model1(self):
        ...
        documents = [
            ...
            "Luxury cars are high-end vehicles designed with superior craftsmanship, advanced technology, and exceptional comfort to offer an elevated driving experience. They often feature premium materials, cutting-edge performance, and exclusive designs, catering to a sophisticated and affluent clientele.",
        ]
    ...
    def test_model2(self):
        ...
        # Encoding documents
        documents = [
            "Setting up your Raspberry Pi is easy. First, download Raspberry Pi OS from the official website and use an imaging tool like Balena Etcher to write the OS image onto a microSD card. Once written, insert the card into the Raspberry Pi, connect the monitor, keyboard, and mouse, and power it up.",
            "To install Raspberry Pi OS, begin by downloading the Raspberry Pi Imager. Select the Raspberry Pi OS from the menu and choose the storage device (SD card). Once the installation is complete, insert the card into the Raspberry Pi and boot it up.",
            "Connecting your Raspberry Pi to a monitor requires an HDMI cable. Depending on the model of Raspberry Pi you have, use either the standard HDMI or micro-HDMI port to connect to your display.",
            "The GPIO (General Purpose Input/Output) pins on the Raspberry Pi allow you to interface with external hardware. There are 40 pins on the Raspberry Pi 4, which are split into power, ground, and GPIO signal pins. Refer to the pinout diagram for exact pin usage.",
            "Programming on the Raspberry Pi can be done in various languages such as Python, Java, or C++. The most commonly used language is Python due to its simplicity and compatibility with most Pi projects.",
            "Using the Raspberry Pi camera module involves connecting the camera to the dedicated camera serial interface (CSI) port. Once connected, you can use the `raspistill` and `raspivid` commands to capture images and videos.",
            "For the Raspberry Pi, the recommended power supply is 5V, 3A with a micro-USB or USB-C connector, depending on the model. A stable power source is crucial to prevent crashes or instability.",
            "Raspberry Pi can be used for home automation with platforms like Home Assistant or OpenHAB. You can use the GPIO pins to control lights, fans, and other devices. You'll need to install the appropriate software and configure the devices.",
            "To connect Raspberry Pi to Wi-Fi, click on the Wi-Fi icon in the desktop environment and select your network. Alternatively, you can configure Wi-Fi by editing the `wpa_supplicant.conf` file on the SD card before booting the Pi.",
            "If your Raspberry Pi is running slowly, consider checking the CPU usage and temperature using tools like `htop` or `vcgencmd`. If the issue persists, try optimizing your code, reducing unnecessary processes, or checking for power supply issues."
        ]
